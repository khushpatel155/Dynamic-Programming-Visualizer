...
const HouseRobberTreeVisualization = ({ 
  treeData, 
  currentStep, 
  algorithmType, 
  isVisible, 
  ...
  const getEdgeLabel = (edge) => {
    // Check if this edge represents a ROB or SKIP choice
    const fromNode = treeData.nodes.find(n => n.id === edge.from);
    const toNode = treeData.nodes.find(n => n.id === edge.to);
    
    if (!fromNode || !toNode) return null;
    
    // Determine if this is a ROB or SKIP edge based on the index relationship
    if (toNode.index === fromNode.index + 2) {
      // ROB edge - include the house value
      const houseValue = houses[fromNode.index] || 0;
      return `ROB (+${houseValue})`;
    } else if (toNode.index === fromNode.index + 1) {
      // SKIP edge - always +0
      return 'SKIP (+0)';
    }
      ...
      <div className="p-4 overflow-auto" style={{ minHeight: '400px' }}>
        ...
        >
          ...
          {currentPositionedData.positioned?.map((node) => {
            ...
            // Always show rob(index)=result format when result is available
            const displayText = hasResult ? `rob(${node.index})=${nodeResult}` : `rob(${node.index})`;
...
export default HouseRobberTreeVisualization;